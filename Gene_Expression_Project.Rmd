...
Several DESeq2 functions are utilized to transform the raw read count data and calculating sample distances. A DESeqDataSet is made using the count data and an annotation which consists of the sample names. The dataset is used to transform the data into normalized data by using the vst function to correct for the variation in raw count values between samples. The normalized data is extracted using the assay function. The t-function is used to transpose the matrix of the normalized values and the distance is calculated using the dist function.
```{r}
...
The annotated data is separated into groups depending on whether the sample was obtained from a control or case subject, whether they were on or off medication, and the hippocampal subfield of interest (CA1, CA3 or DG). These groups were labeled accordingly. This separated data was used to plot a graph showing the MDS data.
```{r}
...
A new group was made in the annotation data frame which contains information about the condition of the sample(control or schizophrenic), which hippocampal subfield was examined and whether or not that person uses anti-psychotic medication. This was done to make it easier to compare these different groups.

Using DESeq a comparison was made between the schizophrenic patients who are on anti-psychotic medication and those who are not. A comparison was also made between the schizophrenia group who dont use madiacation and the healthy contol group. These two comparisons were made for each hippocampal subfield using an adjusted p-value of 0.05. The results of these comparisions gives information about the amount of genes that are up or down regulated.
...
For each hippocampal subfield a volcano plot was made using the results from DESeq to show the difference in gene expression between schizophrenic patient who use medication and those who do not.

In a volcano plot all genes are visualized but only the DEG's are highlighted. To determine which gene is a DEG a threshold of 0.05 was set for the adjusted p-value and 2 for the fold-change.
...
```
Figure: Venn diagram showing the overlap of DEG's between schizophrenia patients ON and OFF medication and between schizophrenia patients without mediaction and the healty control for hippocampal subfiel CA1

Figure: Venn diagram showing the overlap of DEG's between schizophrenia patients ON and OFF medication and between schizophrenia patients without mediaction and the healty control for hippocampal subfiel CA3

Figure: Venn diagram showing the overlap of DEG's between schizophrenia patients ON and OFF medication and between schizophrenia patients without mediaction and the healty control for hippocampal subfiel DG
