...
```{r, fig.subcap=c("A. CA1 Control", "B. CA1 Case")}
par(mfcol = c(1,2))
par(cex = 0.8)
boxplot(log2(counts[control_ca1] + 1), outline = T, cex.names = 0.2, col = "green", las = 2)
...
```{r, fig.subcap=c("C. CA3 Control", "D. CA3 Case")}
par(mfcol = c(1,2))
par(cex = 0.8)
...
```{r fig-sub-2, fig.cap = "Figure 1: Raw counts boxplots", fig.subcap = c("CA1 Control", "CA1 Case","CA3 Control", "CA3 Case", "DG Control", "DG Case"), out.width = "50%", fig.height=8}
#par(mfcol = c(3,2))
#par(cex = 0.8)
...
boxplot(log2(counts[control_ca3] + 1), outline = T, cex.names = 0.2, col = "grey", las = 2)
boxplot(log2(counts[case_ca3] + 1), outline = T, cex.names = 0.2, col = "cyan", las = 2)
boxplot(log2(counts[control_dg] + 1), outline = T, col = "red", las = 2)
...
```{r, fig.cap="Figure 8: Barplot showing the total reads per million for each subject in the control group for subfield CA1", fig.dim=16,8, fig.subcap=c("CA1 Control", "CA1 Case", "CA3 Control", "CA3 Case", "DG Control", "DG Case")}
par(mfcol = c(2,3))
...
```{r fig-sub, fig.cap = "Volcano plot showing the gene expression resulting from the comparison of patients on and off drugs"}
deseq.volcano(res = res_CA1_drug, datasetName = "CA1")
...
deseq.volcano(res = res_DG_drug, datasetName = "DG")
```
Figure 18: Volcano plot showing the gene expression of subfield DG resulting from the comparison of patients on and off drugs
