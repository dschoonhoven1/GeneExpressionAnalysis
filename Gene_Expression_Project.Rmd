...
```{r}
barplot(colSums(counts[control_ca1]) / 1e6, col = "green")
barplot(colSums(counts[case_ca1]) / 1e6, col = "blue")
barplot(colSums(counts[control_ca3]) / 1e6, col = "grey", las =2)
barplot(colSums(counts[case_ca3]) / 1e6, col = "cadetblue2")
barplot(colSums(counts[control_dg]) / 1e6, col = "indianred2")
barplot(colSums(counts[case_dg]) / 1e6, col = "khaki2", las = 2)
```
...
annotation1 <- data.frame(subfield = factor(rep(1:2, each=c(13, 13, 12), 
                                          labels = c("CA1", "CA1", "CA3", "CA3", "DG", "DG"))),
                         type = factor(rep(1:2, each=c(13, 13, 12),
                                         labels = c("control", "schizophrenia", "control", "schizophrenia", "control", "schizophrenia"))))
                         
rownames(annotation1) <- names(count_nooutlier)
```
